...
const Login = () => {
  ...
  // ✅ Handle Facebook redirect result when page loads back
  useEffect(() => {
    getRedirectResult(auth)
      .then(async (result) => {
        ...
        try {
          ...
          console.error("Error processing Facebook redirect result:", err);
        } finally {
          setFbLoading(false);
        }
      })
      .catch((error) => {
        if (error.code === "auth/account-exists-with-different-credential") {
          const email = error.customData?.email;
          alert(
            `The email ${email} is already linked to another account (e.g. Google). Please log in with Google instead.`
          );
        } else if (error.code !== "auth/popup-closed-by-user") {
          console.error(
  ...
  // ✅ Facebook now uses redirect instead of popup
  const handleFacebookLogin = async () => {
    if (fbLoading) return;
    setFbLoading(true);
    try {
      await signInWithRedirect(auth, facebookProvider);
      // Page will redirect to Facebook — nothing runs after this line
    } catch (error: any) {
      console.error("Facebook redirect initiation error:", error);
      setFbLoading(false);
    }
  };
        ...
        <div className="max-[767px]:w-full w-[65%] bg-white">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-6">
            ...
            <button
              ...
              onClick={handleFacebookLogin}
              disabled={fbLoading}
              className="w-full mt-4 flex items-center cursor-pointer justify-center gap-3 border border-gray-300 py-3 rounded-[8px] bg-white hover:bg-gray-50 transition duration-200 font-semibold text-basic-dark shadow-sm disabled:opacity-50"
            >
              <img
                src="https://www.svgrepo.com/show/475647/facebook-color.svg"
                alt="Facebook"
